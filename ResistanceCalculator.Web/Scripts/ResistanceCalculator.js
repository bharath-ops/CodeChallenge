...
$("select").change(function () {
    ...
    // this ajax call returns  resistance value as ajax JSON response.
    $.ajax({
        type:"GET",
        url: "/Home/GetResistanceValue?bandAColor="+ $('#drpFirstBand').val() + "&bandBColor=" + $('#drpSecondBand').val() + "&bandCColor=" + $('#drpThirdBand').val() + "&bandDColor=" + $('#drpFourthBand').val(),
        dataType: "json",        
        success: function (result) {          
                $("#divError").empty();
                $("#divResistanceValue").html(result.resistance);        
            
        },
        error: function (result) {
            $("#divResistanceValue").empty();
            $("#divError").html(result.statusText);
        }
